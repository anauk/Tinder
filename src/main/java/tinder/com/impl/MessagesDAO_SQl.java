...
public class MessagesDAO_SQl implements DAO<MessageItem> {
    ...
    public boolean isEmpty() {
        ...
        try {
            ...
            ResultSet resultSet = st.executeQuery("select count(*) from messages");
            while (resultSet.next()) {
    ...
    public List<MessageItemExtra> getByUser(int id1, int id2){
        ArrayList<MessageItemExtra> messeges = new ArrayList<>();
        try{
            PreparedStatement prs = conn.prepareStatement("select * from messages AS msg " +
                    "join users AS user1 on msg.id1 = user1.id" +
                    "join users AS user2 on msg.id2 = user2.id" +
                    "where (id1=? AND id2=?) " +
                    "or (id1=? AND id2=?) order by ASC ");
            ResultSet resultSet = prs.executeQuery();
            while(resultSet.next()){
                MessageItemExtra item = new MessageItemExtra(
                        ...
                        resultSet.getString("messages"),
                        resultSet.getTimestamp("time"),
                        resultSet.getString("user1.name"),
                        resultSet.getString("user2.name"),
                        resultSet.getString("user1.photo")
                );
                messeges.add(item);
            }
        } catch (SQLException e) {
